...
public class FieldCardHolder : MonoBehaviour
{
    ...
    public void Proc(FieldCardData.ProcType procType, CardData usedCard, GenericBody target, GenericBody user, List<EnemyGenericBody> enemies, List<GenericBody> allies, GenericBody healthPool, bool isPlayerTeam)
    {
        //OnAllyAttack
        //FieldCardData.ProcType procType = FieldCardData.ProcType.OnAllyCastSpell;
        //if (isPlayerTeam)
        //procType = GetProcTypeToTest(eventType, isPlayerTeam);

        Debug.Log("processing proc event " + procType);

        //need a foreach procType in a list generated by the GetProcTypeToTest function
        ...
        for (int i = 0; i < currentFieldCards.Count; i++)
        {
            ...
            {
                if(currentFieldCards[i].procTypeSets[x] == procType && isFieldCardOwnedByPlayers[i] == isPlayerTeam)
                {
